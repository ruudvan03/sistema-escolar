    ...
    <div id="app" class="min-h-screen flex">
        ...
        <aside id="sidebar" class="fixed inset-y-0 left-0 z-50 bg-slate-900 text-white transform -translate-x-full md:translate-x-0 md:relative sidebar-transition shadow-2xl flex flex-col w-64 group">
            ...
            <nav class="flex-1 p-4 space-y-2 overflow-y-auto no-scrollbar">
                ...
                @if(Auth::user()->hasRole('Administrador'))
                    ...
                    <a href="{{ route('maestros.index') }}" class="flex items-center p-3 text-slate-300 hover:bg-slate-800 hover:text-white rounded-lg transition-colors {{ request()->routeIs('maestros.*') ? 'bg-slate-800 text-white border-l-4 border-indigo-500' : '' }}">
                        <svg class="w-6 h-6 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg>
                        <span class="ml-3 sidebar-text">Docentes</span>
                    </a>

                    <a href="{{ route('alumnos.index') }}" class="flex items-center p-3 text-slate-300 hover:bg-slate-800 hover:text-white rounded-lg transition-colors {{ request()->routeIs('alumnos.*') ? 'bg-slate-800 text-white border-l-4 border-blue-500' : '' }}">
                        <svg class="w-6 h-6 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 14l9-5-9-5-9 5 9 5z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 14l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14z"></path></svg>
                        <span class="ml-3 sidebar-text">Alumnos</span>
                    </a>

                    <a href="{{ route('grupos.index') }}" class="flex items-center p-3 text-slate-300 hover:bg-slate-800 hover:text-white rounded-lg transition-colors {{ request()->routeIs('grupos.*') ? 'bg-slate-800 text-white border-l-4 border-rose-500' : '' }}">
                    ...
                    <a href="{{ route('users.index') }}" class="flex items-center p-3 text-slate-300 hover:bg-slate-800 hover:text-white rounded-lg transition-colors {{ request()->routeIs('users.*') ? 'bg-slate-800 text-white border-l-4 border-orange-500' : '' }}">
                    ...
                    <a href="{{ route('roles.index') }}" class="flex items-center p-3 text-slate-300 hover:bg-slate-800 hover:text-white rounded-lg transition-colors {{ request()->routeIs('roles.*') ? 'bg-slate-800 text-white border-l-4 border-pink-500' : '' }}">
                        <svg class="w-6 h-6 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"></path></svg>
                        <span class="ml-3 sidebar-text">Roles y Permisos</span>
                    </a>
                @endif
                ...
                @if(Auth::user()->hasAnyRole(['Administrador', 'Maestro']))
                    ...
                    <a href="{{ route('asistencias.index') }}" class="flex items-center p-3 text-slate-300 hover:bg-slate-800 hover:text-white rounded-lg transition-colors {{ request()->routeIs('asistencias.*') ? 'bg-slate-800 text-white border-l-4 border-green-500' : '' }}">
                        ...
                        <span class="ml-3 sidebar-text">Asistencias</span>
                    </a>

                    <a href="{{ route('calificaciones.index') }}" class="flex items-center p-3 text-slate-300 hover:bg-slate-800 hover:text-white rounded-lg transition-colors {{ request()->routeIs('calificaciones.*') ? 'bg-slate-800 text-white border-l-4 border-yellow-500' : '' }}">
                        <svg class="w-6 h-6 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 3.055A9.001 9.001 0 1020.945 13H11V3.055z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20.488 9H15V3.512A9.025 9.025 0 0120.488 9z"></path></svg>
                        <span class="ml-3 sidebar-text">Calificaciones</span>
                    </a>
                @endif
                ...
                @if(Auth::user()->hasRole('Alumno/Tutor'))
                    ...
                    <a href="{{ route('alumno.boleta') }}" class="flex items-center p-3 text-slate-300 hover:bg-slate-800 hover:text-white rounded-lg transition-colors {{ request()->routeIs('alumno.boleta') ? 'bg-blue-600 text-white shadow-lg' : '' }}">
                        ...
                        <span class="ml-3 sidebar-text">Mi Boleta</span>
                    </a>
                @endif
        ...
        <div class="flex-1 flex flex-col min-h-screen transition-all duration-300">
                    ...
                    <div class="relative"> 
                        ...
                        @else
                            <button id="userMenuBtn" class="flex items-center gap-2 text-sm font-medium text-slate-700 hover:text-blue-600 focus:outline-none transition-colors">
                                ...
                                <div class="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 font-bold border border-blue-200 uppercase">
                            ...
                            <div id="userMenuDropdown" class="hidden absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-xl border border-slate-100 py-1 z-50">
                                <div class="px-4 py-2 border-b border-slate-100 text-xs">
                                    <p class="text-slate-400">Rol: {{ Auth::user()->role->nombre_rol ?? 'Usuario' }}</p>
            ...
            <footer class="bg-white border-t border-slate-200 py-4 px-6 text-center text-xs text-slate-400">
                &copy; {{ date('Y') }} {{ config('app.name') }} | Oaxaca, México
            </footer>
        </div>
    </div>
